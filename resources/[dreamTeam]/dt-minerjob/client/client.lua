...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(100)
		if Login then
			QBCore.Functions.TriggerCallback('dreamTeam-miner:getdatabase', function(exp, pickaxe)
	...
	while true do
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z, true) < 1.5) then	
				...
				if (Config.jobrequirement and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobname)) or not Config.jobrequirement then
					...
					if not JobStarted then						
						DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, Locales['start_job'])
					else
						DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, Locales['end_job'])
					end
					...
					if (IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not JobStarted) then
						...
						if PlayerData.charinfo.gender == 0 then 
							...
							JobStarted = true
							exports.pNotify:SendNotification({text = Locales['job_started'], timeout = 4500})
					...
					DrawText3Ds(Config.miner['BossSpawn'].Pos.x, Config.miner['BossSpawn'].Pos.y, Config.miner['BossSpawn'].Pos.z+1.0, Locales['not_started'])
				end
			end
		elseif(GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) < 4.0) and JobStarted then	
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) < 1.5) then	
				...
				if NewPickaxe and not haspickaxe and Pickaxe <= 6 then
					DrawText3Ds(Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z+1.2, Locales['upgrade_pickaxe']..Config.Pickaxe[Pickaxe+1].color..''..Config.Pickaxe[Pickaxe+1].name..'~w~ ('..Config.Pickaxe[Pickaxe+1].price..'~g~$~w~)')
				end
				...
				if not haspickaxe then
					...
					DrawText3Ds(Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z+1.0, Locales['putdown_pickaxe'])
				end
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not haspickaxe then
					...
					exports.pNotify:SendNotification({text = Locales['go_place'], timeout = 4500})
					haspickaxe = true
				...
				elseif (IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false)) and haspickaxe then
					DeleteEntity(PickaxeObject)
					haspickaxe = false
					exports.pNotify:SendNotification({text = Locales['put_down'], timeout = 4500})
					RemoveBlip(MineBlip)
				elseif IsControlJustReleased(0, Keys['G']) and not IsPedInAnyVehicle(ped, false) and not haspickaxe and Pickaxe <= 6 then
					QBCore.Functions.TriggerCallback('dreamTeam-miner:upgrade', function(cb)
						if cb then 
							Pickaxe = Pickaxe + 1
							exports.pNotify:SendNotification({text = Locales['upg_pickaxe'], Config.Pickaxe[Pickaxe].name.." pickaxe", timeout = 4500})
						else
							exports.pNotify:SendNotification({text = Locales['dont_money'], timeout = 4500})
						end
					end, Pickaxe)
				end
			end
		elseif(GetDistanceBetweenCoords(coords,Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z, true) < 4.0) and JobStarted then
			...
			if(GetDistanceBetweenCoords(coords,Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z, true) < 1.5) then
				sleep = 0
				DrawText3Ds(Config.miner['Sell'].Pos.x, Config.miner['Sell'].Pos.y, Config.miner['Sell'].Pos.z+1.0, Locales['sell_resources'])
				...
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
					QBCore.Functions.TriggerCallback('dreamTeam-miner:payout', function(money, amount, experience)
						if money then
							...
							exports.pNotify:SendNotification({text = Locales['dont_sell'], timeout = 4500})
						end
					end, Experience)
				end
			end
		elseif JobStarted and (GetDistanceBetweenCoords(coords,Config.miner['Pickaxeplace'].Pos.x, Config.miner['Pickaxeplace'].Pos.y, Config.miner['Pickaxeplace'].Pos.z, true) > 100.0) then
			exports.pNotify:SendNotification({text = Locales['left_workplace'], timeout = 4500})
			JobStarted = false
			DeleteEntity(PickaxeObject)
			if haspickaxe then
				RemoveBlip(MineBlip)
		...
		while true do
			...
			if(GetDistanceBetweenCoords(coords,Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.x, Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.y, Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.z, false) < 15.0) then	
				...
				if(GetDistanceBetweenCoords(coords,Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.x, Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.y, Config.MinePlaces[Config.Pickaxe[Pickaxe].name].Pos.z, false) < 4.0) then	
					sleep = 0
					DrawText3Ds(coords.x, coords.y, coords.z+1.2, Locales['start_mine'])
					...
					if IsControlJustReleased(0, 38) and not IsPedInAnyVehicle(ped, false) then
						...
						for i=1,Config.Levels[Level].anim,1 do
							...
							TriggerEvent('dreamTeam-miner:sound', 'pickaxe', 0.5)
							Citizen.Wait(2300)
						end
						ClearPedTasks(PlayerPedId())

						
							TriggerEvent('dreamTeam-miner:sound', 'breakrock', 0.3)
						...
						while true do
							...
							local coords = GetEntityCoords(ped)
								if objectCoords1 and (GetDistanceBetweenCoords(coords,objectCoords1.x, objectCoords1.y, objectCoords1.z, false) < 0.5) then	
									DrawText3Ds(objectCoords1.x, objectCoords1.y, objectCoords1.z, Locales['pickup_raw'])
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords1 = nil
										DeleteEntity(rock1)
										TriggerServerEvent('dreamTeam-miner:mine', Pickaxe, Level)
									end
								elseif objectCoords2 and (GetDistanceBetweenCoords(coords,objectCoords2.x, objectCoords2.y, objectCoords2.z, false) < 0.5) then
									DrawText3Ds(objectCoords2.x, objectCoords2.y, objectCoords2.z, Locales['pickup_raw'])
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords2 = nil
										DeleteEntity(rock2)
								...
								elseif objectCoords3 and (GetDistanceBetweenCoords(coords,objectCoords3.x, objectCoords3.y, objectCoords3.z, false) < 0.5) then
									DrawText3Ds(objectCoords3.x, objectCoords3.y, objectCoords3.z, Locales['pickup_raw'])
									if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
										done = done + 1
										objectCoords3 = nil
										DeleteEntity(rock3)
								...
								if done == 3 then
									if not firsthelp then
										exports.pNotify:SendNotification({text = Locales['go_sell'], timeout = 4500})
										firsthelp = true
									end
									break
								end
...
RegisterNetEvent('dreamTeam-miner:sound')
AddEventHandler('dreamTeam-miner:sound', function(name, vol)
    SendNUIMessage({
