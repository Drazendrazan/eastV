...
RegisterNetEvent('dt-towing:openTowingMenu')
AddEventHandler('dt-towing:openTowingMenu', function()
    OpenTowingMenu()
end)
...
function DetachRope()
    if entity1 and entity2 then
        CloseTowingMenu()
        TriggerServerEvent('dt-towing:stopTow')
...
function AttemptAttachRope()
    if entity1 and entity2 then
        ...
        TriggerServerEvent('dt-towing:tow', veh1, veh2)
        CloseTowingMenu()
...
RegisterNetEvent('dt-towing:removeRope')
AddEventHandler('dt-towing:removeRope', function(id, veh1_, veh2_)
    for k, rope in pairs(ropes) do
        if rope.id == id then
...
RegisterNetEvent('dt-towing:makeRope')
AddEventHandler('dt-towing:makeRope', function(veh1_, veh2_, id, owner)
    for k, rope in pairs(ropes) do
        if rope.id == id then
            DeleteRope(rope.rope)
            ropes[k] = nil
