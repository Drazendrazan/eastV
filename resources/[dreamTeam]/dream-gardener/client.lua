...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	...
	if (Config.jobReq and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobName)) then
        print("[Gardener] Player is a gardener")
	elseif not Config.jobReq then
        print("[Gardener] Player is not gardener")
	end
	Login = true
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        local PlayerData = QBCore.Functions.GetPlayerData()
            if (Config.jobReq and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobName)) or not Config.jobReq then
                        ...
                        if (GetDistanceBetweenCoords(coords, Garage.Pos.x, Garage.Pos.y, Garage.Pos.z, true) < 1.2) then
                            if IsPedInAnyVehicle(ped, false) then
                                ...
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if hasCar then
                                        TriggerServerEvent('dream-gardener:returnVehicle', source)
                                        ReturnVehicle()
                            ...
                            elseif not IsPedInAnyVehicle(ped, false) then
                                ...
                                elseif inGarageMenu then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        inGarageMenu = false
                                        if not hasCar then
                                            QBCore.Functions.TriggerCallback('dream-gardener:checkMoney', function(hasMoney)
                                            if hasMoney then
                                                QBCore.Functions.SpawnVehicle(Config.carSpawn['carOut'].Vehicle, function(vehicle)
                                                    Vehicle = vehicle
                                                ...
                                                end, Config.carSpawn['carOut'].Pos, true)
                                                inGarageMenu = false
                                    ...
                                    elseif Paycheck then
                                        if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 1.5) then
                                            ...
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                ...
                                                TriggerServerEvent('dream-gardener:Payout', money)
