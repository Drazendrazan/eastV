...
RegisterNetEvent('qb-trunk:client:KidnapTrunk', function()
    ...
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            QBCore.Functions.Notify("Bu kişiyi sen kaçırmadın", 'error')
        end
    end
end)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify("Bagajdasın", 'success', 4000)
                                TrunkCam(true)
                                isKidnapped = true
                            else
                                QBCore.Functions.Notify("Bagaj kapalı", 'error', 2500)
                            end
                        else
                        ...
                        QBCore.Functions.Notify("Birisi zaten Bagajda", 'error', 2500)
                    end
                else
                    QBCore.Functions.Notify("Zaten bir Bagajdasın", 'error', 2500)
                end
            else
                QBCore.Functions.Notify("Bu Bagaja giremezsin", 'error', 2500)
            end
        end, plate)
    else
        QBCore.Functions.Notify("Bu Bagaja giremezsin", 'error', 2500)
...
RegisterNetEvent('qb-trunk:client:GetIn', function()
    ...
    if closestVehicle ~= 0 then
        ...
        QBCore.Functions.Notify("Araç bulunamadı", 'error', 2500)
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    sleep = 0
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, "[~g~E~w~] Bagajdan çık")
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, "[~g~G~w~] Bagajı kapat")
                        ...
                        DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.5, "[~g~G~w~] Bagajı aç")
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
