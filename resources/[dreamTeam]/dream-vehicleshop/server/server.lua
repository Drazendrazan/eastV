...
if Config.SetDefaultStockEvent then
    RegisterNetEvent("dream-vehicleshop:setDefaultStock", function()
        local newtable = {}
    ...
    AddEventHandler("dream-vehicleshop:setDefaultStock", function()
        local newtable = {}
        for cat,veh in next,Config.Vehicles do
            for i=1,#veh.buttons do
                local currentVehicle = veh.buttons[i]
...
RegisterNetEvent("dream-vehicleshop:removeStock",function(model)
    for i=1,#data do
...
RegisterNetEvent("dream-vehicleshop:setVehicleOwned",function(vehicleData)
    local src = source
