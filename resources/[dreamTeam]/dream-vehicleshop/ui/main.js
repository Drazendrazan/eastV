...
addEventListener("message", (e) => {
  if (e.data.action == "open") {
    ...
    $(".acceleration-line").css(
      "width",
      e.data.vehicleInfo.acceleration * 10 + "%"
    );
    $(".braking-line").css(
      "width",
      (e.data.vehicleInfo.braking / 50) * 100 + "%"
    );
    $(".traction-line").css(
      "width",
      (e.data.vehicleInfo.traction / 50) * 100 + "%"
    );
    ...
    if (vehiclesInfo.title == title) {
      if (vehiclesInfo.title == "daily vehicles") {
        for (const buttons in data.daily) {
          ...
          let html = `
						<div class="category5" data-name="${vehicleBtn.name}" data-model="${
            vehicleBtn.model
          }" data-costs="${vehicleBtn.costs}" data-stock="${
            vehicleBtn.maxStock
          }">
      ...
      for (const buttons in vehiclesInfo.buttons) {
        ...
        if (vehiclesInfo.buttons.length > 5) {
          let html = `
						<div class="category" data-name="${vehicleBtn.name}" data-model="${
            vehicleBtn.model
          }" data-costs="${vehicleBtn.costs}" data-stock="${
            vehicleBtn.maxStock
          }">
...
$(document).on("click", ".color", function () {
  ...
  $.post(
    "https://dream-vehicleshop/changeColor",
    JSON.stringify({ colorR, colorG, colorB })
  );
});
$(document).on("click", ".category, .category5", function () {
  ...
  $.post("https://dream-vehicleshop/spawnVehicle", JSON.stringify({ model }));
});
...
$("#test-accept").click(function () {
  let timer = data.testDrive.testDriveTimer;
  $.post(
    "https://dream-vehicleshop/testDrive",
    JSON.stringify({ timer, details })
  );
});
...
$("#buy").click(function () {
  ...
  $.post("https://dream-vehicleshop/buyVehicle", JSON.stringify({ details }));
});
  ...
  if (key == "Escape") {
    if (!onBuyPage) {
      if (vehiclesCategory) {
        ...
        $.post("https://dream-vehicleshop/deletevehicle", JSON.stringify({}));
      } else {
        $("#vehicle-class-title-container").fadeOut();
        $("#vehicle-class-container").fadeOut();
        $.post(
          "https://dream-vehicleshop/closeVehicleShop",
          JSON.stringify({})
        );
      }
    }
...
$("#hide-button").click(function () {
  if (!hideDisplay) {
    ...
    $.post("https://dream-vehicleshop/zoomCam", JSON.stringify({}));
  } else {
    hideDisplay = false;
    inspect = false;
    $("#hide-button").css("margin-top", "2px");
    $.post("https://dream-vehicleshop/returnCam", JSON.stringify({}));
    setTimeout(() => {
      $("#vehicle-container").fadeIn();
      $("#title-container").fadeIn();
...
//     $.post("https://dream-vehicleshop/changePos",JSON.stringify({data}));
// }
...
document.onmousedown = function (e) {
  if (inspect) {
    if (vehicleDisplay && !down && e.button == 0) {
      down = true;
      $.post("https://dream-vehicleshop/mousedown", JSON.stringify({}));
    }
  }
};
...
document.onmouseup = function (e) {
  if (inspect) {
    if (down && e.button == 0) {
      $.post("https://dream-vehicleshop/mouseup", JSON.stringify({}));
      down = false;
    }
  }
};
...
window.onmousewheel = function (e) {
  if (inspect) {
    ...
    $.post(
      "https://dream-vehicleshop/" + (st < 0 ? "downscroll" : "upscroll"),
      JSON.stringify({})
    );
  }
};
