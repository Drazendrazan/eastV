...
local function CornerHolder()
    ...
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "cornerholder", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-31",
        firstStreet = locationInfo,
        gender = gender,
        automaticGunfire = false,
        ...
        dispatchMessage = "Uyuşturucu satışı", -- message
        job = { "police" } -- jobs that will get the alerts
    })

end

exports('CornerHolder', CornerHolder)
