...
function dropAnim(info,name,key)
...
local weaponDrop = true
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if weaponDrop then
            if IsControlJustReleased(0, vCode.dropKey) then
...
-- RegisterCommand('silahibirak', function()
--     ped = PlayerPedId()
