                ...
                if house ~= nil then
                    ...
                    if result[1] ~= nil then
                        ...
                        MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?',
                            {plantId, house})
                        TriggerClientEvent('QBCore:Notify', src, 'Bitki hasat edildi', 'success', 3500)
                        TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, house)
                    else
                        TriggerClientEvent('QBCore:Notify', src, 'Bitkiden çok uzaksın', 'error', 3500)
                    end
                else
                TriggerClientEvent('QBCore:Notify', src, 'Evde değilsin!', 'error', 3500)
                end
            else
                TriggerClientEvent('QBCore:Notify', src, 'Bitki makasın yok', 'error', 3500)
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Yeterince poşetin yok", 'error', 3500)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Yeterince poşetin yok", 'error', 3500)
    end
...
RegisterNetEvent('qb-weed:server:foodPlant', function(house, amount, plantName, plantId)
    ...
    TriggerClientEvent('QBCore:Notify', src,
        QBWeed.Plants[plantName]["label"] .. ' | Gübre: ' .. plantStats[1].food .. '% + ' .. amount .. '% (' ..
