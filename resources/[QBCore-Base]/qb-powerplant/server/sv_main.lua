...
local function CheckStationHits()
    ...
    if Config.PowerStations[1].hit and Config.PowerStations[2].hit and Config.PowerStations[3].hit and Config.PowerStations[4].hit then
        ...
        TriggerClientEvent('chatMessage', -1, "[LS Water & Power]", "error", "Şehir genelinde elektrik kesintisi, üzerinde çalışıyoruz!")
...
QBCore.Functions.CreateUseableItem("thermite", function(source, item)
	...
	if Player.Functions.GetItemByName('lighter') then
        ...
        TriggerClientEvent('QBCore:Notify', source, "Fitili yakacak çakmağın yok.", "error")
    end
end)
...
CreateThread(function()
    while true do
        ...
        if blackoutActive then
            ...
            TriggerClientEvent('chatMessage', -1, "[LS Water & Power]", "normal", "Şehir genelinde elektrik kesintisi bitti.")
