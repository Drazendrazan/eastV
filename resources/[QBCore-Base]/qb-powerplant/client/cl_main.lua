...
local function ThermitePowerPlantFailed()
    TriggerServerEvent("QBCore:Server:RemoveItem", "thermite", 1)
    TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["thermite"], "remove")
    QBCore.Functions.Notify("Thermite başarısız oldu..", "error")
end
            ...
            QBCore.Functions.TriggerCallback('qb-powerplant:server:getCops', function(cops)
                if cops >= Config.MinimumThermitePolice then
                    if not Config.PowerStations[closestStation].hit then
                        ...
                        QBCore.Functions.Notify("Sigortalar atmış gibi görünüyor..", "error")
                    end
                else
                    QBCore.Functions.Notify('Yeterince polis yok ('.. Config.MinimumThermitePolice ..') Gerekli', 'error')
                end
            end)
        end
    end
