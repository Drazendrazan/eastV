...
RegisterNetEvent('qb-police:client:openPoliceStash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "LSPD Depo", {
        maxweight = 9000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", "LSPD Depo")
end)

RegisterNetEvent('qb-police:client:openDnaStash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Delil Deposu", {
        maxweight = 9000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", "Delil Deposu")
end)
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("policeStash_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-police:client:openPoliceStash",
...
-- Police Evidence Stash Target
if Config.UseTarget then
    CreateThread(function()
        -- Toggle Evidence Stash
        for k, v in pairs(Config.Locations["evidence"]) do
            exports['qb-target']:AddBoxZone("policeEvidence_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-police:client:openDnaStash",
                        icon = " fas fa-sign-in-alt",
                        label = "DNA Deposu",
                        job = "police",
                    },
